...
/**
     * Novo instanciamento da classe para utilização das funções
     * E utilização do json_encode para enviar os dados no formato json
     * Condicionais para reconhecimento das requisições vindas do AJAX
     */

$dataRequest = new DataRequest();

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($_POST['tipo'] === 'fornecedores') {
      $dadosFornecedores = $dataRequest->dadosFornecedores('');
      echo json_encode($dadosFornecedores);
      exit;
    } else if ($_POST['tipo'] === 'clientes'){
        $dadosClientes = $dataRequest->dadosClientes('');
        echo json_encode($dadosClientes);
        exit;
    }else{
